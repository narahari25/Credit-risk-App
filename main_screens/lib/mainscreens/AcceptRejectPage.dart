...
class AcceptRejectPage extends StatefulWidget {
  const AcceptRejectPage({super.key, required this.transactionId});
...
class _AcceptRejectPageState extends State<AcceptRejectPage> {
  ...
  void initState() {
    super.initState();
    print("HOLA");
    _updateTransactionStatus(); // Call update function on initialization
  }

  Future<void> _updateTransactionStatus() async {
    final FirebaseDatabase database = FirebaseDatabase.instance;
    final transactionRef = database.ref().child('transactions/${widget.transactionId}');
    print("HOLA");
    try {
      await transactionRef.update({'accepted': 1});
      setState(() {
  ...
  Widget build(BuildContext context) {
    final Person person = people[0]; // Assuming you have person data
    final Size size = MediaQuery.of(context).size;
    const double padding = 25;
    final FirebaseDatabase database = FirebaseDatabase.instance;

    // if(_isTransactionUpdated)
    // {
    ...
    //           ElevatedButton(
    //             onPressed: () => _updateTransactionStatus(), // Retry button (optional)
    //             child: Text('Retry'),
    //           ),
    //         ],
    //       ),
    ...
    // }
// else {
      // Show loading indicator or initial UI while update is in progress
      return Scaffold(
        body: Center(
